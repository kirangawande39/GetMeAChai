...
import { createPayment } from "@/app/action/paymentAction";
...
export default async function Username({ params }) {
  ...
  const total_amount = supporters.reduce((acc, curr) => acc + curr.amount, 0);
        ...
        <div className="make-payment bg-slate-950 opacity-75 rounded-lg h-auto w-full lg:w-[40vw] p-8">
          ...
          <form  className="gap-5 mt-5">
            ...
            <input
              ...
              // onChange={handleChange}
            />
            <button  className="w-full mt-5 cursor-not-allowed font-bold text-white bg-gradient-to-br from-purple-600 to-blue-500 rounded-lg text-sm px-5 py-2.5">
              Support
            </button>
          </form>
