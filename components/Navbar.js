...
const Navbar = () => {
  ...
  return (
    <nav className="bg-gray-900 text-white flex justify-between items-center px-5 h-[8vh] relative">
        ...
        {session ? (
          <>
            <div className="relative">
              ...
              >
                Welcome {session.user.name} <span className="text-lg">&#9660;</span>
              </button>
              ...
              {isOpen && (
                <div className="absolute right-0 mt-2 bg-slate-200 rounded-lg text-black w-40 z-50 shadow-lg">
                  <div className="flex flex-col p-2 gap-y-1">
                    ...
                    <Link href={`/${session.user.name}`} className="hover:bg-white px-2 rounded-sm">
                      Your Page
                    </Link>
                  </div>
                  <hr />
      ...
      {menuOpen && (
        <div className="absolute top-[8vh] right-0 w-3/4 bg-gray-800 text-white flex flex-col p-4 gap-2 md:hidden z-50 transition-all duration-300">
          {session ? (
            <div className="flex flex-col gap-2">
              <Link href="/dashboard" className="hover:bg-gray-700 px-2 py-1 rounded">
                Dashboard
              </Link>
              <Link href={`/${session.user.name}`} className="hover:bg-gray-700 px-2 py-1 rounded">
                Your Page
              </Link>
              <button
                onClick={() => signOut()}
